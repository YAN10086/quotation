 ...
 * @LastEditTime: 2023-12-04 13:08:59
 */
...
//查询公告
router.post("/selectQue", function (req, res, next) {
  ...
  //查询分页数据
  let sql_text = `SELECT * FROM` + " `que` " + `WHERE ${sqlCondition}  ORDER BY id   limit ${page} ,${reqdata.pageSize}`;
  //查询总数
  let sql_text_count = `SELECT COUNT(*) as totalCount from que WHERE ${sqlCondition}`;
...
router.post("/selectFavGoods", function (req, res, next) {
  let reqdata = req.body;
  console.log("插入参数***********", reqdata); //查询参数
  let sql_text = "SELECT fav.*, goods.* FROM fav JOIN goods ON fav.goodsId = goods.id " + ` WHERE fav.openId ='${reqdata.openId}'  and fav.tenantId ='${reqdata.tenantId}' `;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
      objdata = commonJs.backData(200, "查询成功", results);
