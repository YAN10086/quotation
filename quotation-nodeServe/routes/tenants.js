 ...
 * @LastEditTime: 2024-01-17 01:26:48
 */
...
//查询是否收藏
router.post("/selectTenantsByCode", function (req, res, next) {
  let reqdata = req.body;
  console.log("插入参数***********", reqdata); //查询参数
  let sql_text = `SELECT * FROM tenants  WHERE tenantCode='${reqdata.tenantCode}' and enabled = 1 `;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
