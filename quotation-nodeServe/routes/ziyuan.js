 ...
 * @LastEditTime: 2024-04-30 22:35:05
...
//查询所有商品品牌
router.post("/selectZiyuan", function (req, res, next) {
  var reqdata = req.body;
  var sql_text = `SELECT * from ziyuan `;
...
//根据类型查询商品
router.post("/selectZiyuansByBid", function (req, res, next) {
  var reqdata = req.body;
  var sql_text = `SELECT * from ziyuan WHERE id = '${reqdata.zid}' and enabled = 1`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
      objdata = commonJs.backData(200, "请求成功。", results);
...
//查询已购买的商品
router.post("/selectZiyuansByUser", function (req, res, next) {
  var reqdata = req.body;
  var sql_text = `SELECT * from myziyuan WHERE openid = '${reqdata.openid}' and enabled = 1`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
      objdata = commonJs.backData(200, "请求成功。", results);
...
router.post("/selectFavGoods", function (req, res, next) {
  ...
  let sql_text = "SELECT ziyuan.*, myziyuan.* FROM ziyuan JOIN myziyuan ON myziyuan.ziyuanId = ziyuan.id " + ` WHERE myziyuan.openId ='${reqdata.openId}'  ORDER BY fav.id DESC`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
...
//添加收藏
router.post("/addZiyuan", function (req, res, next) {
  let reqdata = req.body;
  console.log("插入参数***********", reqdata); //查询参数
  let createDate = +new Date();
  let sql_text = "INSERT INTO  myziyuan " + `(openId,ziyuanId,createDate,tenantId) VALUES ('${reqdata.openId}','${reqdata.ziyuanId}','${createDate}','${reqdata.tenantId}')`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
...
//查询是否购买过
router.post("/checkZiyuan", function (req, res, next) {
  let reqdata = req.body;
  console.log("插入参数***********", reqdata); //查询参数
  let sql_text = "SELECT * FROM myziyuan " + ` WHERE openId='${reqdata.openId}' and ziyuanId='${reqdata.ziyuanId}' `;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
      objdata = commonJs.backData(200, "查询成功", results);
