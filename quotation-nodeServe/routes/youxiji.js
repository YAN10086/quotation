 ...
 * @LastEditTime: 2024-04-05 23:13:33
...
router.post("/selectGoodsByBid", function (req, res, next) {
  var reqdata = req.body;
  var sql_text = `SELECT * from youxiji WHERE bid = '${reqdata.bid}' and  tenantId = '${reqdata.tenantId}' and enabled = 1`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
      objdata = commonJs.backData(200, "请求成功。", results);
...
router.post("/selectGoods", function (req, res, next) {
  ...
  //查询分页数据
  let sql_text = `SELECT * FROM` + " `youxiji` " + `WHERE ${sqlCondition}  ORDER BY id   limit ${page} ,${reqdata.pageSize}`;
  //查询总数
  let sql_text_count = `SELECT COUNT(*) as totalCount from youxiji WHERE ${sqlCondition}`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    let objdata;
    let bData = {
...
router.post("/selectRecommendGoods", function (req, res, next) {
  ...
  let page = (reqdata.page - 1) * reqdata.pageSize; //查询起始
  //查询分页数据
  let sql_text = `SELECT * FROM youxiji WHERE ${sqlCondition} ORDER BY visitCount DESC  limit ${page} ,${reqdata.pageSize}`;
  //查询总数
  let sql_text_count = `SELECT COUNT(*) as totalCount FROM youxiji WHERE ${sqlCondition}`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
...
router.post("/updateGoodsInfo", function (req, res, next) {
  ...
  let sql_text = `UPDATE youxiji SET ${sqlCondition} WHERE id = ${reqdata.id}`;
  connection.query(sql_text, function (error, results, fields) {
    console.log("sql_text", sql_text, results);
    if (results) {
...
router.post("/updatePriceBatch", function (req, res, next) {
  ...
  if (reqdata.id) {
    //单个
    sql_text_p = `select * FROM youxiji WHERE id = '${reqdata.id}' and tenantId= '${reqdata.tenantId}'`;
  } else if (reqdata.bid) {
    //多品牌
    // sql_text_p = `select * FROM youxiji WHERE bid in (${reqdata.bid}) and tenantId= '${reqdata.tenantId}'`;
  } else if (reqdata.all) {
    //所有
    sql_text_p = `select * FROM youxiji WHERE   tenantId= '${reqdata.tenantId}'`;
  }
  connection.query(sql_text_p, function (error, results_p, fields) {
    ...
    connection.beginTransaction((err) => {
      ...
      for (let i = 0; i < bodyLen.length; i++) {
        ...
        let sql_text = `UPDATE youxiji SET recyclingPriceList = '${priceList}' , updateDate = '${updateDate}' WHERE id = '${bodyLen[i].id}' and tenantId= '${bodyLen[i].tenantId}'`;
