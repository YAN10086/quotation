 ...
 * @LastEditTime: 2024-01-17 01:45:39
 * @Description: file content
-->
<template>
  <div class="right-main">
    <div class="main-table">
      <el-button type="primary" @click="addVip">添加通知</el-button>
      <el-table
        :data="tableData"
        border
...
export default {
  ...
  methods: {
    ...
    delGoods(index, row) {
      ...
      this.$confirm("将要删除该通知, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(() => {
          that.$api.post("/sys/delNoticeBar", pdata).then(
            res => {
              console.log("res", res);
              if (res.data.code == "200") {
                that.$message({
    ...
    addVip() {
      if (this.total > 0) {
        this.$message({
          message:
            "只允许存在一条通知，目前已存在一条，可编辑现有通知，或者删除现有通知然后新建",
          type: "warning"
        });
        return;
      }
    ...
    getData() {
      ...
      this.loadingTable = true;
      this.$api.post("/sys/getNoticeBar", pdata).then(
        res => {
          console.log("res", res);
          if (res.data.code == "200") {
