		...
		<tn-nav-bar :isBack="false" :bottomShadow="false" backgroundColor="#FFFFFF">
			<view class="custom-nav tn-flex tn-flex-col-center tn-flex-row-left" @click="tn('/minePages/message')">
				<view class="custom-nav__back">
					<text class="tn-text-bold tn-text-xl tn-color-black">公告通知</text>
				</view>
			</view>
		</tn-nav-bar>

		<view class="" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			<view class="tn-margin wallpaper-shadow tn-padding" v-for="(item,index) in noticeList" :key="index">
				<view class="tn-flex tn-flex-col-top">
					<view class="">
						<view class="logo-pic">
							<view class="logo-image">
								<view :class="[item.type == '1' ? 'xt' : 'td']"
									style="width: 100rpx;height: 100rpx;background-size: cover;">
								</view>
							</view>
						</view>
					</view>
					<view class="tn-padding-left-sm" style="width: 100%;">
						<view class="tn-flex tn-flex-row-between tn-flex-col-between">
							<view class="justify-content-item">
								<text class="tn-color-wallpaper tn-text-lg tn-text-bold">{{item.type == '1' ? '系统通知' : '渠道通知'}}</text>
								<text class="tn-color-gray tn-padding-left-sm tn-padding-right-xs">{{item.type == '1' ? '系统' : userInfo.tenantName}}</text>
							</view>
				...
				<view class="tn-flex tn-flex-row-between tn-flex-col-between tn-margin-top-sm">
					...
					<view :class="['justify-content-item tn-round tn-text-xs','bg-blue color-blue']"
						style="padding: 5rpx 15rpx;" v-if="item.flag == '1'" @tap="copyText(item)">
						<text class="tn-padding-right-xs">复制</text>
					</view>
					<view :class="['justify-content-item tn-round tn-text-xs',item.type == '1' ? 'tn-bg-orange--light tn-color-orange' : 'bg-blue color-blue']"
						...
						<text class="tn-padding-right-xs">{{item.type == '1' ? '系统' : '团队'}}</text>
					</view>
				</view>
			</view>
		</view>
